...
-- Temporary table for shortlisting
...
CREATE TABLE profiles (
    ...
    prf_image BYTEA,
    prf_updated DATE DEFAULT CURRENT_DATE
);
...
CREATE TABLE resource_allocations (
    ...
    ral_resource_id INTEGER REFERENCES resources(resource_id) ON DELETE NO ACTION,
    ral_phase_id INTEGER REFERENCES hiring_phases(hph_id) ON DELETE NO ACTION,
    ral_capacity INTEGER,
    ral_occupied INTEGER,
    ral_faculty_id INTEGER REFERENCES faculty(fac_id) ON DELETE SET NULL,
...
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO users(usr_id, usr_password, usr_role)
    VALUES(NEW.roll_no, 'student@123', 'STUD');

    INSERT INTO profiles(prf_usr_id)
    VALUES(NEW.roll_no);
...
BEGIN
    INSERT INTO users(usr_id, usr_password, usr_role)
    VALUES(NEW.po_id, 'admin@123', 'ADMN');
